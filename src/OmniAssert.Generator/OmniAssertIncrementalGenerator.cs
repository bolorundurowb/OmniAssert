...
/// <summary>
/// Emits interceptors for each interceptable <c>Ensure.Expression(bool, string?)</c> or legacy
/// <c>VerifyExpression(bool, string?)</c> call site by default. Bare identifiers redirect to
/// <c>Must(...).BeTrue()</c>; other boolean shapes call <c>Expression(bool, string?)</c> with the captured expression text.
...
#pragma warning restore RS0030
                    ...
                    var extensionCallSite = IsExtensionCallSite(node, method);
                    ...
                    list.Add(new InterceptorCandidate(location, simple, extensionCallSite));
                }
            }
    ...
    private void RegisterRewritePipeline(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(rewriteInputs, (spc, tuple) =>
        {
            ...
            var header = "// <auto-generated/>\n// OmniAssert: rewritten Expression / VerifyExpression call sites with operand capture.\n#nullable enable\n";
            spc.AddSource(
    ...
    private static string EmitInterceptorsFile(string className, List<InterceptorCandidate> candidates)
    {
        ...
        sb.AppendLine("// OmniAssert: intercepts Ensure.Expression(bool, ...) and legacy VerifyExpression call sites.");
        ...
        for (var i = 0; i < candidates.Count; i++)
        {
            ...
            if (candidate.ExtensionCallSite)
            {
            ...
            {
                sb.AppendLine("            global::OmniAssert.Ensure.Expression(condition, expression);");
            }
    ...
    /// <summary>
    /// Returns the boolean expression from an <c>Expression</c> or <c>VerifyExpression</c> invocation regardless of call form.
    /// For extension-method calls (<c>expr.VerifyExpression()</c>) the bool is the receiver;
    /// for static calls (<c>Ensure.Expression(expr)</c> or <c>Assert.VerifyExpression(expr)</c>) it is the first argument.
    /// </summary>
    ...
    /// <summary>
    /// <c>true</c> for extension call sites (<c>expr.VerifyExpression()</c>); <c>false</c> for static invocations.
    /// </summary>
    private static bool IsExtensionCallSite(InvocationExpressionSyntax node, IMethodSymbol method)
    {
